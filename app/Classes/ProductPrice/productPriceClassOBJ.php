...
Class productPriceClassOBJ {
    ...
    public function getProductPrice() {
        ...
        if (!is_null($productPrice)) {
            $util = ['melyik' => 'lastProductPrice', 'ar' => (float)($productPrice->Price), 'offerOverride' => 0];
        ...
        if (count($contractPrice) > 0) {
            for ($i = 0; $i < count($contractPrice); $i++) {
                $util = ['melyik' => 'contractPrice', 'ar' => (float)($contractPrice[$i]->Price), 'offerOverride' => $contractPrice[$i]->OfferOverride];
                array_push($this->priceArray, $util);
            }
        }
        ...
        if (!is_null($offerPrice)) {
            $util = ['melyik' => 'offerPrice', 'ar' => (float)($offerPrice->SalesPrice), 'offerOverride' => 0];
            array_push($this->priceArray, $util);
        }
