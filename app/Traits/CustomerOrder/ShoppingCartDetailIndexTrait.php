    ...
    public function shoppingCartDetailIndex(Request $request, $id)
    {
        if( myUser::check() ){
            ...
            if ($request->ajax()) {
                ...
                $data = DB::table('shoppingcartdetail as t1')
                    ->select('t1.*', 't2.Name as CurrencyName', 't3.Name as ProductName', 't4.Name as QuantityUnitName', 't5.Name as VatName', 't5.Rate as VatRate', 't7.Name as StatusName')
                    ...
                    ->leftJoin('customerorderdetail as t6', 't6.Id', '=', 't1.CustomerOrderDetail')
                    ->leftJoin('customerorderdetailstatus as t7', 't7.Id', '=', 't6.DetailStatus')
                    ->where('t1.ShoppingCart', $id)
                    ->get();
                ...
                return Datatables::of($data)
                    ...
                    ->addColumn('StatusName', function($data) { return is_null($data->StatusName) ? 'aaaa' : $data->StatusName; })
                    ->make(true);
